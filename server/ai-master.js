...
const SKILLS = {
  acrobatics: 'dex', animal_handling: 'wis', arcana: 'int', athletics: 'str', deception: 'cha', history: 'int', insight: 'wis', intimidation: 'cha', investigation: 'int', medicine: 'wis', nature: 'int', perception: 'wis', performance: 'cha', persuasion: 'cha', religion: 'int', sleight_of_hand: 'dex', stealth: 'dex', survival: 'wis'
};
...
function normalizeRoll(character, pending) {
  ...
  const proficient = skill ? Boolean(character.skill_proficiencies?.[skill]) : false;
  ...
  return {
    check_type: pending.check_type || 'ABILITY_CHECK',
    ability,
    skill,
    dc,
    modifier,
    notation: `1d20${modifier >= 0 ? '+' : ''}${modifier}`,
    reason: String(pending.reason || 'Игровая проверка').slice(0, 200)
  };
}
...
async function contextFor(characterId, campaignId) {
  ...
  const [messages, events, locations, bestiary, quests] = await Promise.all([
    ...
    pool.query(`SELECT id,name,description,knowledge_level FROM (SELECT l.*,k.level AS knowledge_level FROM locations l JOIN knowledge_entries k ON k.subject_id=l.id WHERE l.campaign_id=$1::uuid AND k.campaign_id=$1::uuid AND k.character_id=$2::uuid AND k.subject_type='LOCATION' AND k.level<>'UNKNOWN') x ORDER BY name`, [campaignId, characterId]),
...
async function askModel(messages) {
  ...
  const response = await fetch(`${config.aiBaseUrl.replace(/\/$/, '')}/chat/completions`, {
    ...
    body: JSON.stringify({ model: config.aiModel, messages, temperature: 0.75, max_tokens: 1400 })
  });
...
async function executeAction({ campaignId, characterId, action }) {
  ...
  if (action.type === 'USE_ITEM') {
    return useItem({ campaignId, actorId: characterId, entryId: action.inventory_entry_id, targetId: action.target_character_id || characterId, context: action.context || 'Использовано во время приключения' });
  }
  if (action.type === 'DISCOVER_LOCATION') {
    return discoverLocation({ campaignId, characterId, locationId: action.location_id, level: action.level || 'SEEN', facts: Array.isArray(action.facts) ? action.facts : [] });
  }
  if (action.type === 'DISCOVER_CREATURE') {
    return discoverCreature({ campaignId, characterId, creatureId: action.creature_id, level: action.level || 'SEEN', facts: Array.isArray(action.facts) ? action.facts : [] });
  }
...
export async function handlePlayerMessage({ campaignId, characterId, body }) {
  ...
  const result = await askModel([
    ...
    { role: 'system', content: `АКТУАЛЬНОЕ СОСТОЯНИЕ ИГРЫ:\n${JSON.stringify(context)}` },
...
export async function handleRoll({ campaignId, characterId, pendingRoll }) {
  const context = await contextFor(characterId, campaignId);
  const rollSpec = normalizeRoll(context.character, pendingRoll);
  ...
  const followup = await askModel([
    { role: 'system', content: RULES },
    { role: 'system', content: `АКТУАЛЬНОЕ СОСТОЯНИЕ ИГРЫ:\n${JSON.stringify(context)}` },
    { role: 'system', content: `ИГРОВОЙ ДВИЖОК УЖЕ ВЫПОЛНИЛ БРОСОК. Нельзя менять результат. Проверка: ${JSON.stringify(rollSpec)}. Выпало на d20: ${rolled.diceTotal}. Итог: ${total}. Успех: ${success}. Теперь опиши последствия. Если требуется подтверждённое игровое действие, верни action; новый бросок не запрашивай.` },
    { role: 'user', content: 'Продолжи сцену после результата проверки.' }
  ]);
  let actionResult = null;
