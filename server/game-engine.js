...
export async function applyDamage({ campaignId, sessionId, actorId, targetId, amount, damageType = 'untyped' }) {
  return transaction(async (client) => {
    ...
    const target = rows[0];
    if (!target) throw Object.assign(new Error('Персонаж не найден'), { status: 404 });
    const incoming = Math.max(0, Number(amount));
    const absorbed = Math.min(target.temp_hp, incoming);
    const hpDamage = incoming - absorbed;
    const hp = Math.max(0, target.hp - hpDamage);
    const tempHp = target.temp_hp - absorbed;
    ...
    const concentration = await client.query('SELECT id FROM effects WHERE source_character_id=$1 AND concentration AND active', [targetId]);
...
export async function applyHealing({ campaignId, sessionId, actorId, targetId, amount, temporary = false }) {
  return transaction(async (client) => {
    ...
    const hp = temporary ? target.hp : Math.min(target.hp_max, target.hp + value);
    const tempHp = temporary ? Math.max(target.temp_hp, value) : target.temp_hp;
    await client.query('UPDATE characters SET hp=$1,temp_hp=$2,updated_at=now() WHERE id=$3', [hp, tempHp, targetId]);
...
export async function addEffect(input) {
  return transaction(async (client) => {
    if (input.concentration && input.sourceId) await client.query('UPDATE effects SET active=false WHERE source_character_id=$1 AND concentration AND active', [input.sourceId]);
    const { rows } = await client.query(
...
export async function identifyItem({ campaignId, characterId, entryId, method, propertyKey }) {
  return transaction(async (client) => {
    ...
    const property = rows[0].properties.find((value) => value.key === propertyKey);
...
export async function recruit({ campaignId, actorId, characterId, type = 'INVITED', wageGp = 0 }) {
  return transaction(async (client) => {
    const found = await client.query('SELECT id,name FROM characters WHERE id=$1 AND campaign_id=$2', [characterId,campaignId]);
    if (!found.rows[0]) throw Object.assign(new Error('Кандидат не найден'), { status: 404 });
