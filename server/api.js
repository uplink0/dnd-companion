...
api.get('/bootstrap', asyncRoute(async (req,res) => {
  ...
  const [campaign, character, party, quests, locations, messages, journal, bestiary] = await Promise.all([
    pool.query('SELECT * FROM campaigns WHERE id=$1', [campaignId]), characterSummary(characterId),
    ...
    pool.query('SELECT * FROM messages WHERE campaign_id=$1 ORDER BY created_at DESC LIMIT 60', [campaignId]),
...
api.get('/campaigns/:id/events', asyncRoute(async (req,res) => res.json((await pool.query('SELECT * FROM game_events WHERE campaign_id=$1 AND sequence>$2 ORDER BY sequence LIMIT 100',[req.params.id,Number(req.query.after||0)])).rows)));
...
api.post('/campaigns/:id/messages', asyncRoute(async (req,res) => {
  ...
  const result = await transaction(async (client) => {
    ...
    const answer = `Вы заявили: «${input.body}». Мастер учитывает состояние героя, активные эффекты, окружение и последствия предыдущих событий. Если действие рискованно — сделайте подходящую проверку.`;
    const master = (await client.query(`INSERT INTO messages(campaign_id,role,body,metadata) VALUES($1,'MASTER',$2,$3) RETURNING *`,[req.params.id,answer,{ engine:'local-rules' }])).rows[0];
