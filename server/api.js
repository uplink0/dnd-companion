...
api.get('/campaigns/:id/characters', asyncRoute(async (req, res) => {
  const { rows } = await pool.query(`
    ...
    ORDER BY created_at,name`, [req.params.id]);
...
api.get('/bootstrap', asyncRoute(async (req, res) => {
  ...
  const [campaign, character, party, quests, locations, messages, journal, events, bestiary] = await Promise.all([
    ...
    pool.query(`
      ...
      WHERE campaign_id=$1 AND ($2='' OR character_id=$2 OR (role='SYSTEM' AND character_id IS NULL))
      ORDER BY created_at DESC LIMIT 60`, [campaignId, characterId]),
    pool.query(`
      SELECT * FROM journal_entries
      WHERE campaign_id=$1 AND ($2='' OR author_character_id=$2 OR author_character_id IS NULL)
    ...
    characterId
      ? pool.query(`
          ...
          WHERE k.campaign_id=$1 AND k.subject_type='CREATURE' AND k.level<>'UNKNOWN' AND k.character_id=$2`, [campaignId, characterId])
...
api.get('/campaigns/:id/quests', asyncRoute(async (req, res) => {
  res.json((await pool.query(
    'SELECT * FROM quests WHERE campaign_id=$1 ORDER BY status,title',
    [req.params.id]
  )).rows);
}));
...
api.post('/campaigns/:id/rolls', asyncRoute(async (req, res) => {
  ...
  await assertCampaignCharacter(pool, input.characterId, req.params.id);
  ...
  const { rows } = await pool.query(`
    ...
    [req.params.id,input.characterId,roll.notation,roll.diceTotal,roll.modifier,roll.total,input.reason || '']
  );
...
api.post('/campaigns/:id/messages', asyncRoute(async (req, res) => {
  ...
  const result = await transaction(async (client) => {
    const character = await assertCampaignCharacter(client, input.characterId, req.params.id);
    const player = (await client.query(
      `INSERT INTO messages(campaign_id,character_id,role,body) VALUES($1,$2,'PLAYER',$3) RETURNING *`,
      [req.params.id,input.characterId,input.body]
    )).rows[0];
    const recent = (await client.query(
      `SELECT body,role FROM messages WHERE campaign_id=$1 AND character_id=$2 ORDER BY created_at DESC LIMIT 6`,
      [req.params.id,input.characterId]
    )).rows.reverse();
    ...
    const master = (await client.query(
      `INSERT INTO messages(campaign_id,character_id,role,body,metadata) VALUES($1,$2,'MASTER',$3,$4) RETURNING *`,
      [req.params.id,input.characterId,answer,{engine:'local-rules',recentTurns:recent.length}]
    )).rows[0];
    await client.query(
      ...
      [req.params.id,input.characterId,player.id,{text:input.body,responseId:master.id}]
    );
...
api.post('/campaigns/:id/damage', asyncRoute(async (req,res) => {
  ...
  res.json(await applyDamage({ campaignId:req.params.id, ...input }));
}));
...
api.post('/campaigns/:id/healing', asyncRoute(async (req,res) => {
  ...
  res.json(await applyHealing({ campaignId:req.params.id, ...input }));
}));
...
api.post('/campaigns/:id/effects', asyncRoute(async (req,res) => {
  ...
  res.status(201).json(await addEffect({ campaignId:req.params.id, ...input }));
}));
...
api.post('/campaigns/:id/quests/:questId/accept', asyncRoute(async (req,res) => {
  ...
  res.json(await acceptQuest({ campaignId:req.params.id, questId:req.params.questId, characterId }));
}));
...
api.post('/campaigns/:id/items/:entryId/identify', asyncRoute(async (req,res) => {
  ...
  res.json(await identifyItem({ campaignId:req.params.id, entryId:req.params.entryId, ...input }));
}));
...
api.post('/campaigns/:id/party/recruit', asyncRoute(async (req,res) => {
  ...
  res.status(201).json(await recruit({ campaignId:req.params.id, ...input }));
}));
